...
export const PrometheusFormSection: React.FC<{
  ...
  // Rule group state — synced to parent via onUpdate
  const [ruleGroupName, setRuleGroupName] = useState(form.name || '');

  const handleRuleGroupChange = useCallback(
    (value: string) => {
      setRuleGroupName(value);
      // Propagate to parent form so it's included on submission
      onUpdate('name' as keyof PrometheusFormState, value as any);
    },
    [onUpdate]
  );
  ...
  // Sync builder selections to PromQL query — only when builder fields change
  ...
  return (
    <>
                ...
                <EuiSelect
                  options={[
                    ...datasources
                      ...
                      .map((ds) => ({ value: ds.id, text: ds.name })),
                    ...(datasourceId && !datasources.find((ds) => ds.id === datasourceId)
      ...
      <EuiPanel paddingSize="m" color="subdued">
        ...
        <EuiText size="xs" color="subdued">
          Alert rules are organized into groups. Rules in the same group share an evaluation
          interval and are evaluated together.
        </EuiText>
        <EuiSpacer size="s" />
        <EuiFormRow
          label="Group name"
