...
export const PrometheusFormSection: React.FC<{
  ...
  useEffect(() => {
    if (queryMode === 'builder') syncBuilderToQuery();
  }, [queryMode, syncBuilderToQuery]);
      ...
      <EuiPanel paddingSize="m" color="subdued">
        ...
        >
          <EuiComboBox
            ...
            onChange={(opts) => setRuleGroupName(opts.length > 0 ? opts[0].label : '')}
            onCreateOption={(value) => setRuleGroupName(value)}
            singleSelection={{ asPlainText: true }}
            compressed
            isClearable
